...
function logViewer() {
    return {
        ...
        itemLink(entry) {
            var label = entry.details?.itemLabel;
            if (!label) return null;
            var baseURL = entry.details?.instanceBaseURL || '';
            var path = entry.details?.itemDetailPath || '';
            var url = baseURL && path ? baseURL + path : '';
        ...
        levelClass(level) {
            switch (level) {
                case 'debug':
                    return 'bg-brand-700 text-gray-300';
                case 'info':
                    return 'bg-accent-600/20 text-accent-500';
                case 'warn':
